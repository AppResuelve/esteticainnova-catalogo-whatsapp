...
export function ProductGallery({ images, productName, discountPercentage }) {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="aspect-square bg-[var(--color-surface)] rounded-xs flex items-center justify-center">
  ...
  return (
    <div className="space-y-4">
      <div className="relative aspect-square rounded-xs overflow-hidden bg-[var(--color-surface)]">
        <img
        ...
        {discountPercentage && (
          <div className="absolute top-3 right-3">
            <span className="inline-flex items-center px-3 py-1.5 rounded-xs text-sm font-bold bg-[var(--color-secondary)] text-yellow-800">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {images.map((image, index) => (
            <button
              ...
              className={`shrink-0 w-20 h-20 rounded-xs overflow-hidden border-2 transition-colors ${
                index === currentIndex

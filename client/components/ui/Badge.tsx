export function Badge({ children, variant = 'default', color = undefined, className = '' }) {
  const variants = {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-xs text-sm font-semibold border ${color ? '' : variants[variant]} ${className}`}
      style={colorStyle}
    >
      {children}
    </span>

...
function Pagination({ page, totalPages, onPageChange }) {
  ...
  const btnBase =
    "w-9 h-9 flex items-center justify-center rounded-full text-sm font-medium transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed";
  const btnActive =
    "bg-[var(--color-primary)] text-white shadow-[0_4px_12px_rgba(203,110,228,0.3)]";
  const btnInactive =
...
export default function ProductsClient({
  ...
  const categories =
    ctxCategories.length > 0 ? ctxCategories : serverCategories;
  ...
  return (
    <>
      {/* ══ HERO ══ */}
      <section className="relative overflow-hidden px-4 sm:px-6 lg:px-6 pt-20 pb-4">
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            backgroundImage: 'url("/headercatalogo.jpeg")',
